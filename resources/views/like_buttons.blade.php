<div class="flex">
    <form method="POST" action="/beeps/{{ $beep->id }}/like">
    @csrf

        <div class="flex items-center mr-4 {{ $beep->isLikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
    ...
    @method('DELETE')
        ...
        <div class="flex items-center {{ $beep->isDislikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
            <svg viewBox="0 0 20 20" class="mr-1 w-3">
                <g id="Page-1"
                   stroke="none"
                   stroke-width="1"
